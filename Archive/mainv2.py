...
def checkSleep(bedbbox,bedCounter,bb,consecBedIouTracker,solidDetections):
	for i in range (len(bedbbox)):
		iou = calculateIou(bedbbox[i],bb)


		if iou > 0:
			consecBedIouTracker[i].pop(0)
			consecBedIouTracker[i].append(iou)
			flagIncreasing = True
			flagDecreasing = True
			prev = 0
			for j in range(len(consecBedIouTracker[i])):
				if prev == 0:
					prev = consecBedIouTracker[i][j]
				else:
					if prev - consecBedIouTracker[i][j] > 0:
						flagIncreasing = False
					if prev - consecBedIouTracker[i][j]	< 0:
						flagDecreasing = False
					prev = consecBedIouTracker[i][j]

			if flagIncreasing and bedCounter[i] < solidDetections:
				bedCounter[i] += 1
			if flagDecreasing and bedCounter[i] > 0:
				bedCounter[i] -= 1

	print('consecBedIouTracker: ')
	print(consecBedIouTracker)
	print('bedcounter: ')
	print(bedCounter)
...
def startStream(args):
	...
	# Used to track number of people on the bed.
	bedCounter = [0]
	# Used to track total bed time 
	bedTime = [0]
	# Used to see the previous count of consecutive movement, if the queue got 4 consective increases we then add a person to the bed.
	consecBedIouTracker = [[0,0,0,0]]
	# Used to track total number of solid detections so we do not overcount
	solidDetections = 0
		...
		if totalFrames%20 == 0:
			...
			for i in np.arange(0, num_objects):
				...
				if confidence[i] > args["confidence"] and classes[i] == 0:
					...
					checkSleep(bedbbox,bedCounter,bb,consecBedIouTracker,solidDetections)
		...
		# multiple objects
			...
			for oq in outputQueues:
				...
				if buffer2 == ('person', (0,0,0,0)):
					...
					# Detect falls here
					colour = fallAlgorithm(startX,startY,endX,endY)
					checkSleep(bedbbox,bedCounter,buffer2[1],consecBedIouTracker,solidDetections)
					cv2.rectangle(frame, (startX, startY), (endX, endY),
						colour, 2)
					cv2.putText(frame, label+ " " + str(outputQueues.index(oq)), (startX, startY - 15),
						cv2.FONT_HERSHEY_SIMPLEX, 0.45, colour, 2)
